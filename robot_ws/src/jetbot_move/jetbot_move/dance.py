...
class Dance(Node):
    ...
    /dance/demo -- Will pull the routine from /src/jetbot_move/routines folder in this ROS package.
    ...
    Examples:
    rostopic pub /dance/demo std_msgs/String start -- This command will use the routine: {ROS_PACKAGE_SHARE}/routines/autumn.json at timestamp 1607551421.
    ...
    def __init__(self):
        ...
        self.get_logger().info(" -- Velocity: %s" % self.get_parameter('cmd_vel_topic')._value)
        self.get_logger().info(" -- String: %s" % self.get_parameter('cmd_str_topic')._value)
    ...
    def start_demo(self, data):
        # Start a new demo dance based on local routines in /src/jetbot_move/routines.
        if (not self.dancing):
            routine = data.data
            self.get_logger().info('Dance demo message recieved. Looking up routine: %s' % routine)
            routines_path = "%s/routines/%s.json" % (get_package_share_directory('jetbot_move'), routine)
